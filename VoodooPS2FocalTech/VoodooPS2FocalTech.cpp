...
ApplePS2FocalTechTouchPad* ApplePS2FocalTechTouchPad::probe(IOService * provider, SInt32 * score)
{
    ...
    if(bytes.byte0 == 0x36 && bytes.byte1 == 0x53 && bytes.byte2 == 0x03)
        success = true;
...
void ApplePS2FocalTechTouchPad::sendTouchDataToMultiTouchInterface() {
    ...
    if(State.pressed){
        unsigned int btn_x,btn_y,finger_num;
        finger_num=0;
        btn_x=0;
        btn_y=0;
        
        for(int i=0;i < FOCALTECH_MAX_FINGERS; i++){
            focaltech_finger_state *fstate = State.fingers+i;
            if(fstate->active&&fstate->valid){
                finger_num++;
                btn_x = clamp(fstate->x, 0U, LOGICAL_MAX_X);
                btn_y = LOGICAL_MAX_Y - clamp(fstate->y, 0U, LOGICAL_MAX_Y);
            }
        }
        if(finger_num == 1 && btn_y > 1400){
            //IOLog("FocaltechButtonXY: [%d][%d]\n",btn_x,btn_y);
            //mid button
            if(btn_x > 1100 && btn_x < 1300){
                buttons = 4;
            }
            if(btn_x >1300){
                //right button
...
void ApplePS2FocalTechTouchPad::getProductID(FTE_BYTES_t *bytes){
    ...
    bytes->byte0 = request.commands[10].inOrOut; // 0x00
    bytes->byte1 = request.commands[11].inOrOut; // 0x13
    bytes->byte2 = request.commands[12].inOrOut; // 0x0d
